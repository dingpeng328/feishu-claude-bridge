...
回复展示使用飞书原生 CardKit Markdown 流：`status_md` 独立显示“⏳ 正在处理”、工具调用总数、完成数、进行中数量和最多两个经过脱敏映射的当前工具名称，`stream_md` 只接收单调增长的回答正文。工具状态变化不会再改写正文元素，因此不会让已经显示的内容回退并重新播放打字动画；卡片也不会展示工具参数、内部路径或逐条调用日志。完成后状态区明确切换为“✅ 回复完成”，显示本轮工具调用总数，以及从消息开始调度到最终结束的处理耗时；标题层级会适配聊天卡片，单张卡片接近 30000 字符时由 bridge 管理续卡并为每张卡维护独立递增序列。失败或中断也会显示明确终态、工具总数及处理耗时，内部错误只保留在服务日志中。流结束后，bridge 会按 `card_id + sequence` 对消息引用的同一个 CardKit 实体强制写入最终状态并回读校验；旧卡片才回退到 `message_id` 更新。若回读暂未看到终态，只会延迟重试更新同一张原卡片，不会新增一条重复回复。

补拉需要在飞书开放平台为机器人应用开通 **获取群组中所有消息**（`im:message.group_msg`）权限；没有该权限时，程序会记录一次明确告警并停用本进程的 HTTP 补拉，WS 接收和定期主动重连不受影响。

### 任务与回复恢复

- 卡片在等待 Agent 并发名额时已可见；话题消息会先读取历史以排除已经完成的重复请求。处理状态每 15 秒刷新耗时和最近收到的 Agent 活动时间；没有输出时明确显示“尚未收到 Agent 输出”，不把计时器当成执行进度。
- 流式正文采用固定分段边界；最终正文允许修订、缩短，并覆盖所有已有卡片。每张卡的状态和完整正文都必须通过回读验证，只有首卡出现“回复完成”不算交付成功。
- 未完成任务及原卡片实体保存在 `~/.feishu-claude-bridge/delivery-state.json.pending.json`，文件权限为 `0600`。最终正文先落盘，再写飞书；暂时交付失败时，每 15 秒重试原卡片，不重新运行 Agent。
- 重启后，已经保存最终正文的任务继续交付；其他未完成任务标记为中断，由用户重新 @ 继续。CLI 会话仍保持“重启后不复用旧 session”的边界。
- 如果发送请求已经发起，但未拿到或未保存 `message_id`，会更新记录中的同一 CardKit 实体并保留待确认记录，日志提示 `original card delivery unconfirmed`；不会因回执不确定而补发第二条回复。
- 收到 SIGINT/SIGTERM 后停止接单，中断本轮 Agent 及其进程组，最多等待 30 秒让卡片收尾；整个退出流程上限为 45 秒。未交付完的卡片留待下次启动恢复。
- 网络恢复重连失败后会以 1、2、4 秒递增退避，最高 30 秒继续尝试。会话文件的并发写入已串行化，避免多个话题同时保存时争用临时文件。
...
│   ├── channel.ts          Channel SDK WS 入站 + CardKit 传输适配
│   ├── cardStream.ts       双元素 CardKit 流 + 长回复续卡/序列管理
